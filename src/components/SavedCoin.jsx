...
const SavedCoin = () => {
  ...
  return (
    <div>
      {coins?.length === 0 ? (<p>Você não tem nenhuma moeda guardada,
        Salve uma moeda para adicioná-la à sua lista.
        <Link to="/"> Clique aqui para pesquisar moedas.</Link></p>) : (
          <table className='w-full border border-collapse text-center'>
            <thead>
              <tr className='border-b'>
                <th className='px-4'>Rank #</th>
                <th className='text-left'>Moeda</th>
                <th className='text-left'>Remover</th>
              </tr>
            </thead>
            <tbody>
              {coins?.map((coin) => (
